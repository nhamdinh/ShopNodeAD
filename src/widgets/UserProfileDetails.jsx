...
const UserProfileDetails = ({ userInfo, avatar }) => {
  ...
  const [updateProfile, { isLoading: isLoading1, error: error1 }] =
    useUpdateProfileMutation();
      ...
      .then((res) => {
        ...
        if (data) {
          ...
          dispatch(
            openToast({
              isOpen: Date.now(),
              content: res?.error?.data?.message ?? "Update Password Failed !",
              step: 2,
            })
          );
        }
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="grid gap-4 md:grid-cols-2 md:gap-5">
            <div className="grid gap-4">
              ...
              <div className="field-wrapper">
                <label className="field-label" htmlFor="passwordOld">
                  Old Password{" "}
                </label>
                <input
                  className={classNames("field-input", {
                    "field-input--error": errors.passwordOld,
                  })}
                  type="text"
                  id="passwordOld"
